...
export default defineConfig({
  base: "./",
  plugins: [errorMonitorPlugin(), react(), createServeGeneratedCssPlugin() ],
  server: {
    port: 3000,
    allowedHosts: true,
  },
...
export function errorMonitorPlugin(): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use("/__healthcheck", (_req, res) => {
...
function createServeGeneratedCssPlugin(): Plugin {
  return {
    ...
    generateBundle(this) {
      const generatedDir = resolve(/* `__dirname` is a global variable in Node.js that represents the
      directory name of the current module. It provides the full path
      to the directory containing the currently executing script. This
      can be useful for resolving relative paths or working with files
